...
```{r}
master <- rio::import("https://raw.githubusercontent.com/Arkansascovid/Main/master/MasterData/master_file.csv")
```
...
```{r}
month <- master %>% 
  select(county_nam, date, New_Cases_Today, month) %>% 
...
monthly_cases <- master %>% 
  select(county_nam, month, year, New_Cases_Today, New_Deaths_Today) %>% 
  filter(county_nam=="Arkansas_all_counties") %>% 
  group_by(month, year) %>% 
...
#Deaths
```{r}
monthly_deaths <- master %>% 
  select(county_nam, month, year, New_Deaths_Today) %>% 
  filter(county_nam=="Arkansas_all_counties") %>% 
  group_by(year, month) %>% 
  summarise(month_deaths = sum(New_Deaths_Today, na.rm = TRUE)) %>% 
  arrange(month)
...
```{r}
monthly_deaths1 <- master %>% 
  select(county_nam, month, year, New_Deaths_Today) %>% 
  filter(county_nam=="Arkansas_all_counties") %>% 
  filter(month=="Jul")

monthly_deaths1
#write.csv(monthly_deaths1, "monthly_deaths1.csv")
```
...
monthly_cases <- monthly_cases %>% 
  mutate(cases_diff_monthly = (month_newcases-lead(month_newcases))) %>% 
   mutate(cases_pct_monthly = (month_newcases-lead(month_newcases))/lead(month_newcases)) 
...
monthly_cases$cases_pct_monthly <- formattable::percent(monthly_cases$cases_pct_monthly)
...
```{r}
deaths <- master %>% 
  select(mydate, county_nam, deaths, New_Deaths_Today, confirmed_death, probable_death, Pct_Deaths_vs_Recoveries, Pct_Recoveries_vs_Deaths )
head(deaths)
...
```{r}
mon <- master %>% 
  select(county_nam, date, New_Cases_Today, weekday) %>% 
  filter(county_nam=="Arkansas_all_counties") %>% 
...
ggsave("monday top cases 11_16.png", device = "png", width=10,height=8, dpi=800)
```
...
```{r}
deaths <- master2 %>% 
  select(mydate, county_nam, New_Deaths_Today) %>% 
  filter(mydate >= ("2020-10-08")) %>% 
  filter(county_nam =="Arkansas_all_counties") %>% 
  summarize(total= sum(New_Deaths_Today))
```
...
```{r}
monthly_deaths <- master %>% 
  select(county_nam, month, New_Deaths_Today) %>% 
  filter(county_nam=="Arkansas_all_counties") %>% 
  group_by(month) %>% 
  summarise(month_deaths = sum(New_Deaths_Today, na.rm = TRUE)) %>% 
  arrange(desc(month)) %>% 
  mutate(deaths_diff_monthly = (month_deaths-lead(month_deaths))) %>% 
  mutate(deaths_pct_monthly = (month_deaths-lead(month_deaths))/lead(month_deaths)) 
...
```{r}
weekly <- master %>% 
  select("county_nam","week", "New_Cases_Today", "pop_5yr_est_2018", "date" ) %>% 
filter(county_nam == "Arkansas_all_counties")  %>% 
#  group_by(week) %>% 
#added population to this able
group_by(week, pop_5yr_est_2018) %>% 
summarise(week_newcases = sum(New_Cases_Today, na.rm = TRUE)) %>% 
#arrange(desc(week_newcases))
arrange(desc(week))
weekly
```
...
```{r}
weekly <- weekly %>% 
  mutate(Cases_Per_100k = ((week_newcases / pop_5yr_est_2018)*100000)) %>% 
arrange(desc(week))
weekly
```
...
#Your Turn
#Deaths on Saturdays: A table just with date, New Deaths
```{r}
sat <- master %>% 
   filter(county_nam=="Arkansas_all_counties") %>% 
   select(date, New_Cases_Today, weekday) %>% 
  filter(weekday == "Saturday") %>% 
  top_n(5, New_Cases_Today) %>% 
  arrange(desc(New_Cases_Today))
...
sat <- master %>% 
   filter(county_nam=="Arkansas_all_counties") %>% 
   select(weekday, date, New_Cases_Today) %>% 
  filter(weekday == "Saturday") %>% 
  top_n(5, New_Cases_Today) %>% 
  arrange(desc(New_Cases_Today))
...
#Graphics
```{r}
ggplot(sat, aes(x=date, y=New_Deaths_Today, color = New_Deaths_Today, fill=New_Deaths_Today)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1.2, hjust = 2.1)) +
  labs(y="Deaths Per Day", title="Arkansas COVID-19 Deaths on Saturdays", caption="Source: ADH")+
  theme(legend.position="none") 

```

#Average Cases Per Day
#The governor has been focusing his comments on beating the previous weeks numbers. Let's get some averages by weekday since August
...
```{r}
weekavg <- weekavg %>% 
  select(weekday, New_Cases_Today) %>% 
  group_by(weekday) %>% 
  summarize(Weekday_Avg=mean(New_Cases_Today))
...
```{r}
# monthly2 <- master %>% 
#   filter(date =="2020-10-31") | filter(date =="2020-09-30")
# 
# df <- master
# 
 # monthly2 <- master %>% 
 #   filter((date == "2020-10-31") | (date == "2020-09-30"))
df2 <- with(df, df[(date == "2020-10-31") | (date == "2020-09-30") | (date == "2020-08-31") | (date == "2020-07-31") | date ==("2020-06-30") | date ==("2020-05-31") | date ==("2020-04-30") | date==("2020-03-31"), ])

monthly3 <- df2 %>% 
  filter(county_nam=="Arkansas_all_counties") %>% 
   mutate(cases_diff_monthly = (confirmed_pos-lead(confirmed_pos))) %>% 
   mutate(cases_pct_monthly = (confirmed_pos-lead(confirmed_pos))/lead(confirmed_pos)) %>% 
  select(county_nam, mydate, positive, confirmed_pos, cases_diff_monthly, cases_pct_monthly)  
...
#new deaths
monthly <- master %>% 
  select(county_nam, month, New_Cases_Today, New_Deaths_Today) %>% 
  filter(county_nam=="Arkansas_all_counties") %>% 
  group_by(month) %>% 
    summarise(month_newdeaths) = sum(New_Deaths_Today, na.rm = TRUE) 
...
```{r}
test8 <- master %>% 
  select(county_nam, mydate, New_Cases_Today, positive, pop_5yr_est_2018) %>% 
  mutate(PCT_Cases_Population = (positive / pop_5yr_est_2018)*100) %>% 
  mutate(New_Cases_Today_100k_Pop = (New_Cases_Today/pop_5yr_est_2018)*10000) %>% 
  filter(county_nam=="Arkansas_all_counties")
test8
```
