...
#Match names
colnames(yesterday_county)[23] <- "recoveries"
colnames(yesterday_county)[31] <- "pop_5yr_est_2018"


names(yesterday_county)
```
# .
# COUNTY DATA SECTION 
#  .
...
countytemp$mydate <- as.Date(countytemp$mydate)
countytemp$pop_5yr_est_2018 <- as.numeric(countytemp$pop_5yr_est_2018)
```
...
#.
#.
# IMPORT THE DAILY HOSPITAL DATA FROM THE GOOGLE Form

```{r}
url <- ("https://docs.google.com/spreadsheets/d/1ikblX8tikM59ma1AftkqgGbyeZkXB6DuBtwMsVeoGYw/edit#gid=701586163")

hospital <- read_sheet(url, sheet = "Hospital Data",  col_types = NULL, na = "",trim_ws = TRUE)
#clean names
...
#Need to Import and Create Hospital Master File to Run Hospital Calculations
#Create a temporary version of master file with hospital data for Sept 18 and add it back later
Match names
```{r}
colnames(hospital) [1] <- "Date"
colnames(hospital) [2] <- "Hospitalized"
colnames(hospital) [3] <- "Total_Beds_Available"
colnames(hospital) [4] <- "Total_ICU_Beds"
colnames(hospital) [5] <- "Total_ICU_Beds_Available"
colnames(hospital) [6] <- "Total_Vents"
colnames(hospital) [7] <- "Total_Vents_Available"
colnames(hospital) [8] <- "Total_Covid_In_ICU"
colnames(hospital) [9] <- "Vent"
...
```{r}
master_hospital <- rio::import("all_state_and_hospitals.csv")
master_hospital <- master_hospital [ -c(1) ]
master_hospital$Date <- as.Date(master_hospital$Date)
glimpse(master_hospital)
```
...
df <- left_join(master_hospital, hospital1, by=c("Date", "Hospitalized", "Hosp_Change_from_Yesterday"))


glimpse(df)
```

#If the data below isn't ready, then you can do a temporary update with this table

```{r}
hospital_master1 <- df
write.csv(hospital_master1, "hospital_master1.csv")

```


#Ever Hospitalized Equations
#Requires Ever_Hospitalized from the Tweet
```{r}
hospital2 <- hospital %>% 
  ...
  mutate(Pct_Hospitalized = (Hospitalized/confirmed_active)*100) %>% 
  mutate(New_on_Vent = (Ever_on_a_Vent-lead(Ever_on_a_Vent))) 

hospital2 <- hospital2 %>% 
  select(Date, Hospitalized, New_Admits, New_Discharges_Deaths, Pct_Hospitalized, New_on_Vent)
```

#Join hospital2 calcs
```{r}
all_state_and_hospitals <- left_join(master_hospital, hospital2, by=c("Date", "Hospitalized"))
