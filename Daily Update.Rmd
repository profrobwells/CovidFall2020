...
top_numbers <- master2 %>% 
  filter(county_nam=="Arkansas_all_counties") %>% 
    filter(mydate>=(yesterday)) %>% 
  select(mydate, New_Cases_Today, positive,  deaths, New_Deaths_Today, active_cases, recoveries, Recovered_Since_Yesterday)

top_numbers$Date <- as.character((top_numbers$mydate), format='%m%-%d%-%Y')

top_numbers$positive <- scales::comma(top_numbers$positive)
top_numbers$active_cases <- scales::comma(top_numbers$active_cases)
top_numbers$recoveries <- scales::comma(top_numbers$recoveries)
top_numbers$deaths <- scales::comma(top_numbers$deaths)
top_numbers$New_Cases_Today <- scales::comma(top_numbers$New_Cases_Today)
top_numbers$Recovered_Since_Yesterday <- scales::comma(top_numbers$Recovered_Since_Yesterday)

top_numbers <- top_numbers %>% 
  rename('Positive Cases' = positive, Recoveries = recoveries, Deaths = deaths, 'Active Cases' = active_cases, 'New Cases Today' = New_Cases_Today, 'Recovered Since Yesterday' = Recovered_Since_Yesterday, 'New Deaths Today' = New_Deaths_Today)

top_numbers <- top_numbers %>% 
  select(Date, 'Positive Cases', Recoveries, Deaths, 'Active Cases', 'New Cases Today', 'Recovered Since Yesterday', 'New Deaths Today')
...
t_export <-  gt(t) %>%
  tab_header(
    title = md("**The COVID-19 Story in Arkansas Today**"))
...
t_export <- t_export %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(Value, Today, Yesterday))
  ) %>%  
  cols_width(
    vars(Value) ~ px(300),
    everything() ~ px(170))

gtsave(t_export, "top_numbers.png")
```
...
scale_y_continuous(limits=c(0, 4500))  +
  ...
  labs(title = "New Cases & Recoveries in Arkansas Announced Today", 
       subtitle = "Confirmed + Probable,  ADH Data for Jan. 7, 2021",
...
```{r}
#hospital <- rio::import("/Users/rswells/Dropbox/Classes/Data Fall 2020/ArkansasCovid/Hospital Data - Manual Input - Hospital-TWEET SLIDE.csv")
```
...
#Vaccine Data
# IMPORT THE Vaccine Data FROM THE GOOGLE Form

```{r include=FALSE}
#url3 <- ("https://docs.google.com/spreadsheets/d/1ikblX8tikM59ma1AftkqgGbyeZkXB6DuBtwMsVeoGYw/edit?usp=sharing")

vaccine <- read_sheet(url3, sheet = "Vaccinations",  col_types = NULL, na = "",trim_ws = TRUE)
```

```{r}
#clean names
vaccine <- janitor::clean_names(vaccine)
vaccine <- vaccine %>% 
  select(date, doses_given, doses_received,federal_l_t_care_doses_allocated, federal_l_t_care_doses_given)
colnames(vaccine)[1:5] <- c("Date", "Doses_Given", "Doses_Received", "Long_Term_Care_Doses", "Long_Term_Care_Given")
glimpse(vaccine)
```
#math
```{r}
vaccine <- vaccine %>% 
  mutate(All_Doses_Received = (Doses_Received + Long_Term_Care_Doses)) %>% 
  mutate(All_Doses_Given = (Doses_Given + Long_Term_Care_Given)) %>% 
