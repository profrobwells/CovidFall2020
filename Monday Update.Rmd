...
Sat_New_Cases_Today <- c(760)
Sat_New_Deaths_Today <- c(1)
Sat_active_cases <- c(6995)
Sat_deaths <- c(5949)
Sat_positive <- c(356220)
mydate <- as.Date(c('2021-07-10'))
...
Sun_New_Cases_Today <- c(965)
Sun_New_Deaths_Today <- c(0)
Sun_active_cases <- c(7528)
Sun_deaths <- c(5949)
Sun_positive <- c(357185)
mydate <- as.Date(c('2021-07-11'))
...
# ```{r}
# #GOOGLE FORM! REPLACE THE NUMBERS WITH THE Monday ADH DATA
# 
# New_Cases_Today <- c(238)
# New_Deaths_Today <- c(1)
# active_cases <- c(4752)
# deaths <- c(5920)
# positive <- c(351825)
# mydate <- as.Date(c('2021-07-05'))
# 
# mon_daily <- data.frame(mydate, New_Cases_Today, New_Deaths_Today, active_cases, positive, deaths)
# 
# mon_daily <- mon_daily %>%
...
## County Level Arkansas Covid Data from ADH FEED
```{r}
#New County json feed
#38 Variables
qq <- fromJSON('https://services5.arcgis.com/vlhGQkz6fSnofVMD/ArcGIS/rest/services/ADH_COVID19_STATS_BY_COUNTY/FeatureServer/0/query?where=0%3D0&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&resultType=none&distance=0.0&units=esriSRUnit_Meter&returnGeodetic=false&outFields=*&returnGeometry=true&returnCentroid=false&featureEncoding=esriDefault&multipatchOption=xyFootprint&maxAllowableOffset=&geometryPrecision=&outSR=&datumTransformation=&applyVCSProjection=false&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&returnQueryGeometry=false&returnDistinctValues=false&cacheHint=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&having=&resultOffset=&resultRecordCount=&returnZ=false&returnM=false&returnExceededLimitFeatures=true&quantizationParameters=&sqlFormat=none&f=pjson&token=')

df33 <- qq[["features"]][["attributes"]]
#print's today's date as a date field
df33$mydate <- Sys.Date()
...
#today_county_new <- df33
today_county <- df33

today_county <- today_county %>% 
  select(county_nam, Pop_5YR_EST_2018,positive,negative,pending,lab_prvt,
lab_pub,deaths,total_tests,pos_range,
neg_range,active_cases,confirmed_pos,probable_pos,
confirmed_recov,probable_recov,confirmed_death,probable_death,
confirmed_active,probable_active,pcr_pos_test,pcr_neg_test,
antigen_pos_test,antigen_neg_test,total_pos_test,total_neg_test,
POP_5YR_EST_2018_2019, antigen_test,pcr_test,recoveries,
Shape__Area,Shape__Length,mydate)
```
...
today_county [2:33] <- lapply(today_county [2:33], as.numeric)
...
```
#Weekend Totals Added to yesterday_county 

```{r}
# 
weekend <- rbind(sat_daily, sun_daily)
#
weekend <- weekend %>%
     arrange(desc(Date))
...
weekend <- weekend %>%
  rename(New_Cases_Today = 'New Cases Today', New_Deaths_Today = 'New Deaths Today', active_cases = 'Active Cases', positive = Positive, deaths = Deaths, mydate = Date)

weekend <- weekend %>% 
  mutate(county_nam = 'Arkansas_all_counties')
...
yesterday_county <- smartbind(yesterday_county, weekend)

yesterday_county$mydate <- ymd(yesterday_county$mydate)
...
```{r}
#For Friday, specify the date
# countytemp <- yesterday_county %>% 
#   filter(mydate>=("2021-07-02")) %>%  
#   arrange(county_nam) 
...
#No Recovered Since Yesterday Calculation
```{r}
twodays <- twodays %>%
...
```{r}
twodays <- twodays %>% filter(mydate > friday) 
```
...
#New Monday Case Slide Graphic
```{r}

monday_slide <- test2 %>% 
  select(mydate, New_Cases_Today,New_Cases_7_Day_Avg, Recovered_Since_Yesterday) %>% 
  rename('New Cases' = New_Cases_Today, Recoveries = Recovered_Since_Yesterday)

monday_slide$New_Cases_7_Day_Avg <- round(monday_slide$New_Cases_7_Day_Avg,0)

write.csv(monday_slide, "monday_slide.csv")
#Feeds this slide
#https://app.flourish.studio/visualisation/6656839/edit?
  
```


#New Monday Death Slide Graphic
```{r}

monday_death_slide <- test2 %>% 
  select(mydate, New_Deaths_Today,New_Deaths_7_Day_Avg) %>% 
  rename('New Deaths' = New_Deaths_Today)

monday_death_slide$New_Deaths_7_Day_Avg <- round(monday_death_slide$New_Deaths_7_Day_Avg,0)

write.csv(monday_death_slide, "monday_death_slide.csv")
#Feeds this slide
#https://app.flourish.studio/visualisation/6656839/edit?
...
#Redo cases, deaths slide to compare three days to three days
...
#Table with 2021, 2020 years separated
```{r}
newcases <- rbind(newcases_2021, newcases_2020) 
```  
...
```{r}
#Join with cases; manually change date filter after a week
weeklycases_2020 <- newcases_2020 %>% 
  right_join(weeks_numbers, by=c("week"="Number")) %>% 
  filter(Date < "2021-01-04") %>% 
  distinct() %>% 
  arrange(desc(Date))

weeklycases_2021 <- newcases_2021 %>% 
  right_join(weeks_numbers, by=c("week"="Number")) %>% 
  filter(Date<today) %>% 
  distinct() %>% 
  arrange(desc(Date))

weeklycases_2021 <- weeklycases_2021 %>% 
  filter(Date > "2021-01-04") 

weeklycases <- rbind(weeklycases_2021, weeklycases_2020 )
...
weekend1 <- weekend %>%
  rename('New Cases Today' = New_Cases_Today, 'New Deaths Today'= New_Deaths_Today, 'Active Cases' = active_cases, Positive = positive, Deaths = deaths, Date = mydate)
...
```{r}
last_monday_county <- yesterday_county %>% 
  filter(mydate==(today-7)) %>% 
  arrange(county_nam) 

last_monday_county[4:45] <- lapply(last_monday_county[4:45], as.numeric)
```
# .
# Previous Date Calculations
...
You should get a df with 154 observations (two days' worth a data)
```{r}
monday_master <- smartbind(today_county, last_monday_county)
...
- **IMPORTANT: FILTER TABLE TO TODAY'S RESULTS**
```{r}
monday_twodays <- monday_twodays %>% filter(mydate == today) #SHOULD GET 77 ROWS IN twodays

monday_weekly_total <- monday_twodays %>% 
  select(county_nam, New_Cases_Today, Recovered_Since_Yesterday, New_Deaths_Today) %>% 
  rename(County = county_nam, Weekly_Case_Total = New_Cases_Today, Recovered_Weekly = Recovered_Since_Yesterday, Deaths_Weekly = New_Deaths_Today)

write.csv(monday_weekly_total, "monday_weektotals.csv")
```
