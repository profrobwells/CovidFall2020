...
# df3 <- rio::import("/Users/rswells/Dropbox/Classes/Data Fall 2020/ArkansasCovid/CovidFall2020/ADH_COVID19_TESTING_COUNTY_DAILY-3_26.xlsx", which="3_25")
...
#It feeds this Flourish graphic: https://app.flourish.studio/visualisation/4895391/edit 
```
...
scale_y_continuous(limits=c(0, 500))  +
  ...
  labs(title = "New Cases & Recoveries in Arkansas Announced Today", 
       subtitle = "Confirmed + Probable,  ADH Data for April 28, 2021",
...
## Retrieve Vaccine Data from ADH FEED
```{r}
#New County json feed
#6 Variables
qq <- fromJSON('https://services.arcgis.com/PwY9ZuZRDiI5nXUB/ArcGIS/rest/services/ADH_COVID19_VACCINATIONS_ADMIN_DIST/FeatureServer/0/query?where=0%3D0&objectIds=&time=&resultType=none&outFields=*&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnDistinctValues=false&cacheHint=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&having=&resultOffset=&resultRecordCount=&sqlFormat=none&f=pjson&token=')
...
vax1 <- clean_names(vax1)

colnames(vax1)[1:7] <- c("All_Doses_Given", "All_Doses_Received", "Long_Term_Care_Doses", "Doses_Received", "Long_Term_Care_Given", "Doses_Given", "Date")
vax1 [1:6] <- lapply(vax1 [1:6], as.character)
vax1 [1:6] <- lapply(vax1 [1:6], as.numeric)
...
```{r include=FALSE}
vax2 <- rio::import("https://raw.githubusercontent.com/Arkansascovid/Main/master/vaccine.csv")
...
pp <- fromJSON('https://services.arcgis.com/PwY9ZuZRDiI5nXUB/arcgis/rest/services/ADH_COVID19_VACCINATIONS_BY_HAA/FeatureServer/0/query?where=0%3D0&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&resultType=none&distance=0.0&units=esriSRUnit_Meter&returnGeodetic=false&outFields=*&returnGeometry=true&returnCentroid=false&featureEncoding=esriDefault&multipatchOption=xyFootprint&maxAllowableOffset=&geometryPrecision=&outSR=&datumTransformation=&applyVCSProjection=false&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&returnQueryGeometry=false&returnDistinctValues=false&cacheHint=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&having=&resultOffset=&resultRecordCount=&returnZ=false&returnM=false&returnExceededLimitFeatures=true&quantizationParameters=&sqlFormat=none&f=pjson&token=')

#Pulls in the data that matters to 'p'
vax_haa <- pp[["features"]][["attributes"]]

#Rename Variables

colnames(vax_haa)[1:23] <-c("objectid","Health_Region","objectid2", "haa_regions", "All_Doses_Given_Region", 
 "White", "Black","Native_American","Asian","Pacific_Islander", 
 "Unk_Race", "Other_Race","Population", "Dose_1_Total", "Dose_2_Total", "Unknown_Dose_Total", "Hispanic","Dose_1_Pct","Dose_2_Pct","All_Doses_Pct_Population", 
"Hispanic_Pct", "shape_area", "shape_length")

vax_haa$date <- Sys.Date()
glimpse(vax_haa)
write.csv(vax_haa, "vax_haa.csv")
```
```{r}
race3 <- rio::import("https://raw.githubusercontent.com/Arkansascovid/Main/master/race_main_file.csv")
race <- race3 %>% 
  select(Total_State_Population_African_American, Total_State_Population_Asian, Total_State_Population_Hispanic, Total_State_Population_Multi_Racial, Total_State_Population_White, Total_State_Population_Other, Total_State_Population_Native_American, Total_State_Population_Pacific_Islander, Total_State_Population_Race_Or_Ethnicity_Unknown, Pct_State_Population_African_American, Pct_State_Population_Asian, Pct_State_Population_White, Pct_State_Population_Other, Pct_State_Population_Pacific_Islander, Pct_State_Population_African_American, Pct_State_Population_Hispanic, Pct_State_Population_Multi_Racial, Pct_State_Population_Native_American, Pct_State_Population_Race_Or_Ethnicity_Unknown)
...
```{r}
vax_haa_total <- vax_haa %>%
    summarize_if(is.numeric, sum, na.rm=TRUE)
vax_haa_total$haa_regions <- "Total"
vax_haa_total$Health_Region <- "Total"
vax_haa_total$date <- Sys.Date()


vax_haa_total <- vax_haa_total %>% 
  select(Health_Region, haa_regions, All_Doses_Given_Region, White, Black, Hispanic, Native_American, Asian, Pacific_Islander, Unk_Race, Other_Race, Population, Dose_1_Total, Dose_2_Total, Unknown_Dose_Total, Hispanic_Pct, date)

vax_haa2 <- smartbind(vax_haa, vax_haa_total)

glimpse(vax_haa2)
```
```{r}
vax_haa_race <- vax_haa_total %>% 
    inner_join(race, by=c("date")) %>% 
  distinct()

glimpse(vax_haa_race)
```

```{r}
vaa2_haa_race <- vax_haa_race %>% 
  mutate(Pct_White_Vax =(White/Total_State_Population_White)) %>% 
  mutate(Pct_Black_Vax =(Black/Total_State_Population_African_American)) %>% 
  mutate(Pct_Hispanic_Vax =(Hispanic/Total_State_Population_Hispanic)) %>%
    mutate(Pct_Native_Am_Vax =(Native_American/Total_State_Population_Native_American)) %>%
    mutate(Pct_Pacific_Island_Vax =(Pacific_Islander/Total_State_Population_Pacific_Islander)) %>% 
      mutate(Pct_Asian_Vax =(Asian/Total_State_Population_Asian)) 

write.csv(vaa2_haa_race, "vaccine_race_april28.csv" )
...
abc <- vaa2_haa_race %>% 
  select(White, Black, Hispanic, Native_American, Asian, Pacific_Islander)

def <- vaa2_haa_race %>% 
  select(Pct_White_Vax, Pct_Black_Vax, Pct_Hispanic_Vax, Pct_Native_Am_Vax, Pct_Asian_Vax, Pct_Pacific_Island_Vax)

#flip
abc1 <- gather(abc, key = "Race_Ethnicity", value = "Overall_Doses", White, Black, Hispanic, Native_American, Asian, Pacific_Islander)


def1 <- gather(def, key = "Race_Ethnicity", value = "Pct_Vax", Pct_White_Vax, Pct_Black_Vax, Pct_Hispanic_Vax, Pct_Native_Am_Vax, Pct_Asian_Vax, Pct_Pacific_Island_Vax)


def2 <- def1 %>%
  mutate(Race_Ethnicity = case_when(
    str_detect(Race_Ethnicity, "Pct_White_Vax") ~ "White",
    str_detect(Race_Ethnicity, "Pct_Black_Vax") ~ "Black",
    str_detect(Race_Ethnicity, "Pct_Hispanic_Vax") ~ "Hispanic",
    str_detect(Race_Ethnicity, "Pct_Native_Am_Vax") ~ "Native_American",
    str_detect(Race_Ethnicity, "Pct_Asian_Vax") ~ "Asian",
    str_detect(Race_Ethnicity, "Pct_Pacific_Island_Vax") ~ "Pacific_Islander",
    TRUE ~ Race_Ethnicity
  ))
  
#join
vaccine_race_slide <- abc1 %>% 
  inner_join(def2, by = "Race_Ethnicity")

vaccine_race_slide$Pct_Vax <- formattable::percent(vaccine_race_slide$Pct_Vax, 2)

vaccine_race_slide <- vaccine_race_slide %>% 
  select(Race_Ethnicity, Overall_Doses, Pct_Vax)

write.csv(vaccine_race_slide, "vaccine_race_slide.csv")
...
```{r}
#For Footer on Graphic
#This excludes XXXXXvaccines that were counted as 'Unknown Race' or 'Other Race'
vax_haa_race$Unk_Race + vax_haa_race$Other_Race 


#Feeds this chart
#https://app.flourish.studio/visualisation/5535824/edit
```
